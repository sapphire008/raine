...
- Source directories (PEP 508 ``@ file:./pkg``, Poetry ``{ path = \"...\" }``, uv ``[tool.uv.sources]`` directory paths) are copied into ``vendors/`` and rewritten as ``file:./vendors/<name>`` requirements.
- Wheel files (PEP 508 ``@ file:...whl`` or source-table paths ending in ``.whl``) are copied into ``vendors/`` and rewritten as ``file:./vendors/<name>.whl`` requirements.
...
| `extra_dependencies` | Extra PEP 508 reqs merged last; local `@ file:...` dirs and wheels are copied into `vendors/` |
